...
public class CrossSectionsFullBandAudio extends TEPerformancePattern {
    ...
    public CrossSectionsFullBandAudio(LX lx) {
...
//                    max(0, xlv - xwv * Math.abs(p.x - xv) / xRange)
            ...
            c = add(c, LXColor.hsb(
                    ...
                    max(0, ylv - ywv * Math.abs(p.y - (yv * bands[binZ])) / yRange)
...
//                    max(0, zlv - zwv * Math.abs(p.z - zv) / zRange)
                    max(0, zlv - zwv * Math.abs(p.z - (zv * bands[binX])) / zRange)
            ));
            colors[p.index] = c;
        }
    }
